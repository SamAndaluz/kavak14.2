...
class ApprovalRequestCustom(models.Model):
    ...
    next_approver_id = fields.Many2one('res.users', string="Siguiente Aprobador", readonly=True, store=True)
    ...
    def action_approve(self, approver=None):
        ...
        if self.category_id.approval_hierarchy:
            ...
            if len(approvers) >= self.category_id.approval_minimum:
                activities = self.sudo()._get_approval_activities()
                activities.unlink()
                self.send_email_notification_done()
    ...
    def _compute_request_status(self):
        for request in self:
            if request.category_id.approval_hierarchy:
                ...
                if status_lst:
                    ...
                    #    status = 'new'
                    elif status_lst.count('approved') >= len(request.approver_ids):
                        status = 'approved'
                    else:
                        status = 'pending'
                else:
