...
class ApprovalRequestCustom(models.Model):
    ...
    purchase_partner_id = fields.Many2one('res.partner', related="purchase_id.partner_id", string="Proveedor", readonly=True)
    ...
    def action_approve(self, approver=None):
        ...
        if self.category_id.approval_hierarchy:
            if len(approvals) == len(self.mapped('approver_ids')):
                if self.purchase_id:
                    self.purchase_id.sudo().button_approve()
        else:
            if len(approvals) >= self.category_id.approval_minimum:
                if self.purchase_id:
                    self.purchase_id.sudo().button_approve()
